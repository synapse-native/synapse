...
typedef struct TraceEvent {
    ...
    union {
        int timestamp;
        CadenaSegura fn_nombre;
        CadenaSegura archivo;
        int linea;
        int valor_entero;
        float valor_decimal;
        CadenaSegura valor_texto;
        CadenaSegura variable;
    } dato;
} TraceEvent;

typedef struct TraceSession {
    CadenaSegura id;
    CadenaSegura programa;
    void* eventos;
    int total_eventos;
    int estado;
} TraceSession;

struct Resultado registrar_evento(struct TraceEvent evento);
struct Resultado trace(CadenaSegura expresion);
struct TraceSession iniciar_sesion(CadenaSegura programa);
struct Resultado finalizar_sesion(void);
void principal(void);
...
#define EVENT_VARIABLE_CHANGE (6)
#define EVENT_CONTRACT_CHECK (7)
#define EVENT_USER_TRACE (8)
#define TRACE_MAX_EVENTOS (50000)
extern struct TraceSession _syn_debug_iniciar_sesion(CadenaSegura programa);
extern struct Resultado _syn_debug_registrar_evento(int tag, CadenaSegura fn_nombre, CadenaSegura archivo, int linea, CadenaSegura variable, int valor_entero, float valor_decimal, CadenaSegura valor_texto);
extern struct Resultado _syn_debug_trace(CadenaSegura expresion_texto, void* valor, CadenaSegura tipo);
extern struct Resultado _syn_debug_finalizar_sesion(void);
extern struct TraceSession _syn_debug_obtener_sesion(void);
struct Resultado registrar_evento(struct TraceEvent evento) {
    { /* unsafe */
        struct TraceEvent* _e = (struct TraceEvent*)evento;
        return _syn_debug_registrar_evento(_e->tag, _e->fn_nombre.datos, _e->archivo.datos, _e->linea, _e->variable.datos, _e->valor_entero, _e->valor_decimal, _e->valor_texto.datos);
    }
}

struct Resultado trace(CadenaSegura expresion) {
    { /* unsafe */
        return _syn_debug_trace(#expr, &expr, typeof(expr));
    }
}

struct TraceSession iniciar_sesion(CadenaSegura programa) {
    struct TraceSession _ret_98 = _syn_debug_iniciar_sesion(programa);
    return _ret_98;
}

struct Resultado finalizar_sesion(void) {
    struct Resultado _ret_104 = _syn_debug_finalizar_sesion();
    return _ret_104;
}

void principal(void) {
    struct TraceEvent evt;
    _simd_detectar();
    evt = (struct TraceEvent){0};
    evt.tag = EVENT_ASSIGNMENT;
    evt.fn_nombre = (CadenaSegura){ .longitud = (int)strlen("principal"), .datos = "principal" };
    evt.archivo = (CadenaSegura){ .longitud = (int)strlen("test.syn"), .datos = "test.syn" };
    evt.linea = 1;
    evt.variable = (CadenaSegura){ .longitud = (int)strlen("x"), .datos = "x" };
    evt.valor_entero = 42;
