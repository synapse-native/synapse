...
| 1.4 | `nucleo/ast_nodes.syn`: estructuras de nodos del AST | Manual 2 | 🔄 parcial — structs D-F1 añadidas (`LiteralNulo`/`ConstructorTipo`/`DeclaracionTipo` en `nucleo/ast_nodes.syn` + espejo `ast_types.h`, F1.2/F1.2b; `SentenciaDelegar`/`LiteralDecimal` + `tipo_param` en `DeclaracionVariable`, F1.2c); resto de nodos en FASE A/F1.4 |
