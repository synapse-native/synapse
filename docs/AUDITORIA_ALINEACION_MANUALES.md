...
| 1.4 | `nucleo/ast_nodes.syn`: estructuras de nodos del AST | Manual 2 | 🔄 parcial — structs D-F1 añadidas (`LiteralNulo`/`ConstructorTipo`/`DeclaracionTipo` en `nucleo/ast_nodes.syn` + espejo `ast_types.h`, F1.2/F1.2b); resto de nodos en FASE A/F1.4 |
...
| 5.2 | Bootstrap 3 etapas (S1 Python, S2 Synapse→Synapse, S3 Synapse→Synapse) | Manual 9 §9.1 | ✅ VERIFICADO (F1.2b) — `build.bat bootstrap-full` S1→S2→S3 OK; commit `44244a4` |
| 5.3 | Determinismo: diff 0 bytes S2 vs S3 | Manual 9 §9.7 | ✅ VERIFICADO (F1.2b) — `diff = 0 bytes` (S2 == S3 byte-identical); commit `44244a4` |
...
>
> **Progreso F1.2 (2026-08-04):** FASE B **completada (F1.2 parser + F1.2b codegen)** — `declaracion_tipo` (alias, ADT con `|`, genéricos `<T,E>`, paréntesis), `nulo` y `tensor(filas, columnas)` ya se parsean en el frontend embebido `_P_*` (structs `LiteralNulo`/`ConstructorTipo`/`DeclaracionTipo`) **y generan C en S2/S3** (`gen_visitar_top_level` con typedef de alias y tagged-union de ADT, `_oo_expr_a_c` → `crear_tensor()` y macro `nulo`, hoisting de tipos); ver filas F1.2 y F1.2b. **Pendientes de FASE B:** `let`/`delegar`/`arc`/`débil`/`@export`, y el mapeo de `rc`/`modulo` (deuda D-F1 parcial, ver nota).
>
> **REGISTRO DE DEUDA (F1.2b — cuándo se resuelve):** detalle completo en `docs/reportes/F1.2b.md` (check 9/9 PASS + tabla de deuda). **D-F1 (BLOQUEANTE F1.2/1.4):** `let`/`delegar` → F1.2c, `arc`/`débil`/`@export` → F1.2d, `rc`/`modulo` → F1.4. **D-2** (ADT genéricos `T/E` → placeholder `void*`, sin instanciación): FASE A + Fase 2. **D-3** (formato `Tensor t;` vs `Tensor t = {0};` S1 vs S2, cosmético): FASE A. **D-4** (contratos `requiere`/`garantiza` no emitidos por el generador embebido, pre-existente): Fase 5 (verificador_formal). **D-5** (cobertura del ME 40%): ampliar harness en F1.2c.
>
