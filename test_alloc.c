...
void* __syn_asignar(int tamano) {
    void* _ret_5 = NULL;
    return _ret_5;
}

void __syn_liberar(void* ptr) {
    return;
}

int principal(void) {
    Tensor t = (Tensor){ .filas = 2, .columnas = 3, .datos = (float*)__syn_asignar(2 * 3 * sizeof(float)) };
    __syn_liberar(t.datos);
    t = (Tensor){ .filas = 4, .columnas = 5, .datos = (float*)__syn_asignar(4 * 5 * sizeof(float)) };
    int _ret_13 = 0;
    return _ret_13;
    __syn_liberar(t.datos);
}
