...
void assert_bool(CadenaSegura nombre, int64_t obtenido, int64_t esperado);
void assert_entero(CadenaSegura nombre, int64_t obtenido, int64_t esperado);
void assert_eq(CadenaSegura nombre, CadenaSegura obtenido, CadenaSegura esperado);
...
void test_a_texto_cero(void);
void test_a_texto_negativo(void);
void test_a_texto_positivo(void);
void test_contiene_falso(void);
void test_contiene_vacio(void);
void test_contiene_verdadero(void);
void test_escapar_json_comillas(void);
void test_escapar_json_slash(void);
void test_indice_encontrado(void);
void test_indice_no_encontrado(void);
void test_leer_bytes_ejemplo(void);
void test_recortar_espacios(void);
void test_reemplazar_basico(void);
void test_reemplazar_multiples(void);
void test_termina_con_verdadero(void);
...
int64_t tests_pasados = 0LL;
int64_t tests_fallidos = 0LL;
CadenaSegura _validar_ruta_segura(CadenaSegura ruta) {
...
void assert_eq(CadenaSegura nombre, CadenaSegura obtenido, CadenaSegura esperado) {
    ...
    if ((str_eq(obtenido, esperado) == 1)) {
        ...
        escribir_linea(concat((CadenaSegura){ .longitud = (int)strlen("  FAIL: "), .datos = "  FAIL: " }, nombre));
        escribir_linea(concat(concat((CadenaSegura){ .longitud = (int)strlen("    Esperado: \""), .datos = "    Esperado: \"" }, esperado), (CadenaSegura){ .longitud = (int)strlen("\""), .datos = "\"" }));
        escribir_linea(concat(concat((CadenaSegura){ .longitud = (int)strlen("    Obtenido: \""), .datos = "    Obtenido: \"" }, obtenido), (CadenaSegura){ .longitud = (int)strlen("\""), .datos = "\"" }));
        tests_fallidos = (tests_fallidos + 1LL);
          /* [Lifetime Scope: exit depth=1] */
    }
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(obtenido);
    _syn_texto_liberar(nombre);
    _syn_texto_liberar(esperado);
}
...
int64_t _principal_impl(void) {
    _simd_detectar();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("=== test_lsp_io.syn — TDD Tests for LSP I/O ==="), .datos = "=== test_lsp_io.syn — TDD Tests for LSP I/O ===" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- a_texto (conversión entero a texto) ---"), .datos = "--- a_texto (conversión entero a texto) ---" });
    test_a_texto_positivo();
    test_a_texto_negativo();
    test_a_texto_cero();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- contiene (búsqueda de subcadena) ---"), .datos = "--- contiene (búsqueda de subcadena) ---" });
    test_contiene_verdadero();
    test_contiene_falso();
    test_contiene_vacio();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- indice_de (posición de subcadena) ---"), .datos = "--- indice_de (posición de subcadena) ---" });
    test_indice_encontrado();
    test_indice_no_encontrado();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- reemplazar (reemplazo de subcadena) ---"), .datos = "--- reemplazar (reemplazo de subcadena) ---" });
    test_reemplazar_basico();
    test_reemplazar_multiples();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- escapar_json (escape de strings JSON) ---"), .datos = "--- escapar_json (escape de strings JSON) ---" });
    test_escapar_json_comillas();
    test_escapar_json_slash();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- termina_con / recortar ---"), .datos = "--- termina_con / recortar ---" });
    test_termina_con_verdadero();
    test_recortar_espacios();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("--- leer_bytes (lectura binaria de stdin) ---"), .datos = "--- leer_bytes (lectura binaria de stdin) ---" });
    test_leer_bytes_ejemplo();
    escribir_linea((CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("=== RESUMEN ==="), .datos = "=== RESUMEN ===" });
    escribir_linea(concat((CadenaSegura){ .longitud = (int)strlen("Pasados: "), .datos = "Pasados: " }, entero_a_texto(tests_pasados)));
    escribir_linea(concat((CadenaSegura){ .longitud = (int)strlen("Fallidos: "), .datos = "Fallidos: " }, entero_a_texto(tests_fallidos)));
    if ((tests_fallidos > 0LL)) {
...
void test_a_texto_cero(void) {
    CadenaSegura resultado = a_texto(0LL);
    assert_eq((CadenaSegura){ .longitud = (int)strlen("a_texto(0)"), .datos = "a_texto(0)" }, resultado, (CadenaSegura){ .longitud = (int)strlen("0"), .datos = "0" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_a_texto_negativo(void) {
    CadenaSegura resultado = a_texto((-999LL));
    assert_eq((CadenaSegura){ .longitud = (int)strlen("a_texto(-999)"), .datos = "a_texto(-999)" }, resultado, (CadenaSegura){ .longitud = (int)strlen("-999"), .datos = "-999" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}
...
void test_a_texto_positivo(void) {
    ...
    assert_eq((CadenaSegura){ .longitud = (int)strlen("a_texto(42)"), .datos = "a_texto(42)" }, resultado, (CadenaSegura){ .longitud = (int)strlen("42"), .datos = "42" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_contiene_falso(void) {
    int64_t resultado = contiene((CadenaSegura){ .longitud = (int)strlen("synapse"), .datos = "synapse" }, (CadenaSegura){ .longitud = (int)strlen("syquex"), .datos = "syquex" });
    assert_bool((CadenaSegura){ .longitud = (int)strlen("contiene('synapse', 'syquex')"), .datos = "contiene('synapse', 'syquex')" }, resultado, 0LL);
      /* [Lifetime Scope: exit depth=0] */
}

void test_contiene_vacio(void) {
    int64_t resultado = contiene((CadenaSegura){ .longitud = (int)strlen("hola"), .datos = "hola" }, (CadenaSegura){ .longitud = (int)strlen(""), .datos = "" });
    assert_bool((CadenaSegura){ .longitud = (int)strlen("contiene('hola', '')"), .datos = "contiene('hola', '')" }, resultado, 1LL);
      /* [Lifetime Scope: exit depth=0] */
}

void test_contiene_verdadero(void) {
    int64_t resultado = contiene((CadenaSegura){ .longitud = (int)strlen("synapse_lsp"), .datos = "synapse_lsp" }, (CadenaSegura){ .longitud = (int)strlen("lsp"), .datos = "lsp" });
    assert_bool((CadenaSegura){ .longitud = (int)strlen("contiene('synapse_lsp', 'lsp')"), .datos = "contiene('synapse_lsp', 'lsp')" }, resultado, 1LL);
      /* [Lifetime Scope: exit depth=0] */
}

void test_escapar_json_comillas(void) {
    CadenaSegura resultado = escapar_json((CadenaSegura){ .longitud = (int)strlen("hola \"mundo\""), .datos = "hola \"mundo\"" });
    assert_eq((CadenaSegura){ .longitud = (int)strlen("escapar_json comillas"), .datos = "escapar_json comillas" }, resultado, (CadenaSegura){ .longitud = (int)strlen("hola \\\"mundo\\\""), .datos = "hola \\\"mundo\\\"" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_escapar_json_slash(void) {
    CadenaSegura resultado = escapar_json((CadenaSegura){ .longitud = (int)strlen("ruta\\archivo"), .datos = "ruta\\archivo" });
    assert_eq((CadenaSegura){ .longitud = (int)strlen("escapar_json backslash"), .datos = "escapar_json backslash" }, resultado, (CadenaSegura){ .longitud = (int)strlen("ruta\\\\archivo"), .datos = "ruta\\\\archivo" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_indice_encontrado(void) {
    int64_t resultado = indice_de((CadenaSegura){ .longitud = (int)strlen("Content-Length: 50"), .datos = "Content-Length: 50" }, (CadenaSegura){ .longitud = (int)strlen("Length"), .datos = "Length" });
    assert_entero((CadenaSegura){ .longitud = (int)strlen("indice_de('Content-Length: 50', 'Length')"), .datos = "indice_de('Content-Length: 50', 'Length')" }, resultado, 8LL);
      /* [Lifetime Scope: exit depth=0] */
}

void test_indice_no_encontrado(void) {
    int64_t resultado = indice_de((CadenaSegura){ .longitud = (int)strlen("hola"), .datos = "hola" }, (CadenaSegura){ .longitud = (int)strlen("z"), .datos = "z" });
    assert_entero((CadenaSegura){ .longitud = (int)strlen("indice_de('hola', 'z')"), .datos = "indice_de('hola', 'z')" }, resultado, (-1LL));
      /* [Lifetime Scope: exit depth=0] */
}

void test_leer_bytes_ejemplo(void) {
    int64_t tests_pasados;
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("  SKIP: leer_bytes (requiere stdin piping)"), .datos = "  SKIP: leer_bytes (requiere stdin piping)" });
    tests_pasados = (tests_pasados + 1LL);
      /* [Lifetime Scope: exit depth=0] */
}

void test_recortar_espacios(void) {
    CadenaSegura resultado = recortar((CadenaSegura){ .longitud = (int)strlen("  hola  "), .datos = "  hola  " });
    assert_eq((CadenaSegura){ .longitud = (int)strlen("recortar('  hola  ')"), .datos = "recortar('  hola  ')" }, resultado, (CadenaSegura){ .longitud = (int)strlen("hola"), .datos = "hola" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_reemplazar_basico(void) {
    CadenaSegura resultado = reemplazar((CadenaSegura){ .longitud = (int)strlen("hola mundo"), .datos = "hola mundo" }, (CadenaSegura){ .longitud = (int)strlen("mundo"), .datos = "mundo" }, (CadenaSegura){ .longitud = (int)strlen("synapse"), .datos = "synapse" });
    assert_eq((CadenaSegura){ .longitud = (int)strlen("reemplazar('hola mundo', 'mundo', 'synapse')"), .datos = "reemplazar('hola mundo', 'mundo', 'synapse')" }, resultado, (CadenaSegura){ .longitud = (int)strlen("hola synapse"), .datos = "hola synapse" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_reemplazar_multiples(void) {
    CadenaSegura resultado = reemplazar((CadenaSegura){ .longitud = (int)strlen("a-b-c"), .datos = "a-b-c" }, (CadenaSegura){ .longitud = (int)strlen("-"), .datos = "-" }, (CadenaSegura){ .longitud = (int)strlen("|"), .datos = "|" });
    assert_eq((CadenaSegura){ .longitud = (int)strlen("reemplazar('a-b-c', '-', '|')"), .datos = "reemplazar('a-b-c', '-', '|')" }, resultado, (CadenaSegura){ .longitud = (int)strlen("a|b|c"), .datos = "a|b|c" });
      /* [Lifetime Scope: exit depth=0] */
    _syn_texto_liberar(resultado);
}

void test_termina_con_verdadero(void) {
    int64_t resultado = termina_con((CadenaSegura){ .longitud = (int)strlen("hola.syn"), .datos = "hola.syn" }, (CadenaSegura){ .longitud = (int)strlen(".syn"), .datos = ".syn" });
    assert_bool((CadenaSegura){ .longitud = (int)strlen("termina_con('hola.syn', '.syn')"), .datos = "termina_con('hola.syn', '.syn')" }, resultado, 1LL);
      /* [Lifetime Scope: exit depth=0] */
}
