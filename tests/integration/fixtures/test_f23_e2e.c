...
int64_t principal(void) {
    _simd_detectar();
    void* r = nulo;
    escribir_linea(entero_a_texto(42LL));
    void* a = nulo;
    escribir_linea(entero_a_texto(100LL));
    WeakRef w = ((WeakRef){0});
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("rc_clean"), .datos = "rc_clean" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("arc_clean"), .datos = "arc_clean" });
    escribir_linea((CadenaSegura){ .longitud = (int)strlen("debil_clean"), .datos = "debil_clean" });
    rc_weak_release(&w);
    if (r) rc_decrementar(r);
    if (a) arc_decrementar(a);
    return 0LL;
