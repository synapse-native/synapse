...
typedef struct Color { int64_t tag; union { int _unidad; } dato; } Color;

void Contador_pon(struct Contador* self, int64_t nv);
int64_t Contador_sumado(struct Contador* self, int64_t n);
int64_t Contador_valor(struct Contador* self);
void __init__(struct Contador* self);
int64_t clasifica(int64_t n);
int64_t doble(int64_t v);
...
int64_t clasifica(int64_t n) {
    {
        int64_t _match_1842710171940046720 = n;
        switch (_match_1842710171940046720) {
            case 0:
            {
                return 100LL;
                break;
...
int64_t principal(void) {
    _simd_detectar();
    struct Contador c = (struct Contador){0};
    escribir_linea(entero_a_texto(Contador_valor(&(c))));
    escribir_linea(entero_a_texto(clasifica(0LL)));
    escribir_linea(entero_a_texto(doble(LIMITE)));
    int64_t m = Contador_sumado(&(c), 5LL);
    escribir_linea(entero_a_texto(m));
    if ((Contador_valor(&(c)) < LIMITE)) {
        escribir_linea((CadenaSegura){ .longitud = (int)strlen("ok"), .datos = "ok" });
          /* [Lifetime Scope: exit depth=1] */
    }
    return 0LL;
      /* [Lifetime Scope: exit depth=0] */
