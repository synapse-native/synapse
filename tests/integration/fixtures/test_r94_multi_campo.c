...
int64_t Punto_sumar(struct Punto* self, int64_t ox, int64_t oy);
int64_t Punto_total(struct Punto* self);
void __init__(struct Punto* self);
...
int64_t principal(void) {
    _simd_detectar();
    struct Punto p = (struct Punto){0};
    p.xx = 3LL;
    p.yy = 5LL;
    escribir_linea(entero_a_texto(Punto_total(&(p))));
    escribir_linea(entero_a_texto(Punto_sumar(&(p), 2LL, 1LL)));
