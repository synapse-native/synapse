...
def test_cache_hit():
    """Compilar dos veces seguidas — la segunda debe ser cache HIT (mas rapida)"""
    ...
    with tempfile.TemporaryDirectory(prefix="cache_audit_") as td:
        ...
        t0 = time.time()
        rc1, out1, err1 = run([PYTHON, "main.py", test_syn, "-o", os.path.join(td, "out1.exe")])
        t1 = time.time()
        t_first = t1 - t0

        t0 = time.time()
        rc2, out2, err2 = run([PYTHON, "main.py", test_syn, "-o", os.path.join(td, "out2.exe")])
        t1 = time.time()
        t_second = t1 - t0
...
def test_cache_stale():
    """Modificar std/ leaf file y detectar STALE en recompilacion"""
...
def main():
    ...
    # Clear cache
    run([PYTHON, "-c", "import os; os.system('rm -rf ~/.synapse/cache 2>/dev/null || del /q %USERPROFILE%/.synapse/cache 2>/dev/null')"])
