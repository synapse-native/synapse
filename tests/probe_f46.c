...
void consumidor(CanalConcurrencia* ch);
...
typedef struct { CanalConcurrencia* v0; } _args_1_t;
static void _wrap_1(void* arg);

extern void _listener_1(void* arg);
...
void consumidor(CanalConcurrencia* ch) {
    int64_t mensaje;
    fibra_crear((void(*)(void*))_listener_1, (void*)(ch), 0);
    return;
      /* [Lifetime Scope: exit depth=0] */
}
...
void principal(void) {
    CanalConcurrencia* ch;
    _simd_detectar();
    ch = canal_crear(10LL);
    _args_1_t* _args_1 = (_args_1_t*)pool_alloc(sizeof(_args_1_t));
    _args_1->v0 = ch;
    fibra_crear(_wrap_1, _args_1, 0);
    canal_enviar(ch, _synapse_box_int(0LL));
    canal_enviar(ch, _synapse_box_int(5LL));
    canal_enviar(ch, _synapse_box_int(0LL));
    canal_enviar(ch, _synapse_box_int(7LL));
    cerrar(ch);
    return;
    canal_destruir(ch);
      /* [Lifetime Scope: exit depth=0] */
...
void _listener_1(void* arg) {
    (void)arg;
    CanalConcurrencia* _canal = (CanalConcurrencia*)arg;
    bool _cerrado;
    int64_t mensaje;
    while (1) {
    mensaje = ({ void* _m = canal_recibir(_canal, &_cerrado); if (_cerrado) break; _synapse_unbox_int(_m); });
    escribir_linea(entero_a_texto(mensaje));
    }
}

static void _wrap_1(void* _arg) {
    _args_1_t* _a = (_args_1_t*)_arg;
    pool_free(_arg);
    consumidor(_a->v0);
}
