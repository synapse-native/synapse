...
int64_t ahora_ms(void);
void dormir_ms(int64_t ms);
void principal(void);

extern int64_t _syn_ahora_ms(void);
extern void _syn_dormir_ms(int64_t ms);
int64_t ahora_ms(void) {
    return _syn_ahora_ms();
...
void principal(void) {
    ...
    if ((fin >= inicio)) {
        printf("%s\
", (CadenaSegura){ .longitud = (int)strlen("tiempo OK"), .datos = "tiempo OK" }.datos);
          /* [Lifetime Scope: exit depth=1] */
    }
    else {
        printf("%s\
", (CadenaSegura){ .longitud = (int)strlen("FALLO tiempo"), .datos = "FALLO tiempo" }.datos);
          /* [Lifetime Scope: exit depth=1] */
    }
    printf("%s\
