...
void productor(struct CanalConcurrencia* ch);
...
void productor(struct CanalConcurrencia* ch) {
    int x;
    x = 42;
    canal_enviar(ch, _synapse_box_int(x));
}
...
void principal(void) {
    struct CanalConcurrencia* ch;
    ch = canal_crear(10);
    synapse_lanzar_hilo((void*(*)(void*))productor(ch), NULL);
    synapse_lanzar_hilo((void*(*)(void*))consumidor(ch), NULL);
    canal_destruir(ch);
}
