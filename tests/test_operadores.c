...
void principal(void) {
    ...
    int b = 20;
    if ((a <= b)) {
        escribir_linea((CadenaSegura){ .longitud = 5, .datos = "OK_LE" });
    }
    if ((a != b)) {
        escribir_linea((CadenaSegura){ .longitud = 5, .datos = "OK_NE" });
    }
    if (((-a) == (-10))) {
        escribir_linea((CadenaSegura){ .longitud = 5, .datos = "OK_UN" });
    }
}
