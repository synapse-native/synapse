...
int64_t aceptar(int64_t fd_servidor);
void cerrar_cliente(int64_t fd_cliente);
void cerrar_servidor(int64_t fd_servidor);
int64_t iniciar_servidor(int64_t puerto);
CadenaSegura leer_peticion(int64_t fd_cliente);
void principal(void);
void responder(int64_t fd_cliente, int64_t codigo, CadenaSegura tipo, CadenaSegura cuerpo);

extern int64_t _syn_servidor_escuchar(int64_t puerto);
extern int64_t _syn_servidor_aceptar(int64_t fd_servidor);
extern CadenaSegura _syn_http_leer_peticion(int64_t fd_cliente);
extern int64_t _syn_http_enviar_respuesta(int64_t fd_cliente, CadenaSegura respuesta);
extern void _syn_http_cerrar_cliente(int64_t fd_cliente);
extern CadenaSegura _syn_http_respuesta_ok(int64_t codigo, char* tipo, char* cuerpo, int64_t lon);
extern int64_t _syn_iniciar_red(void);
extern int64_t _syn_cerrar_socket(int64_t fd);
int64_t aceptar(int64_t fd_servidor) {
    int64_t fd_cliente;
...
CadenaSegura leer_peticion(int64_t fd_cliente) {
    return _syn_http_leer_peticion(fd_cliente);
      /* [Lifetime Scope: exit depth=0] */
}

void principal(void) {
    int64_t fd;
    CadenaSegura peticion = {0};
    _simd_detectar();
    fd = iniciar_servidor(0LL);
    if ((fd >= 0LL)) {
        printf("%s\
", (CadenaSegura){ .longitud = (int)strlen("servidor OK"), .datos = "servidor OK" }.datos);
        _syn_texto_liberar(peticion);
        peticion = leer_peticion(fd);
        printf("%s\
", (CadenaSegura){ .longitud = (int)strlen("leer_peticion OK"), .datos = "leer_peticion OK" }.datos);
        cerrar_servidor(fd);
          /* [Lifetime Scope: exit depth=1] */
    }
    else {
        printf("%s\
", (CadenaSegura){ .longitud = (int)strlen("FALLO servidor"), .datos = "FALLO servidor" }.datos);
          /* [Lifetime Scope: exit depth=1] */
    }
    printf("%s\
...
void responder(int64_t fd_cliente, int64_t codigo, CadenaSegura tipo, CadenaSegura cuerpo) {
    CadenaSegura res = {0};
    _syn_texto_liberar(res);
    res = _syn_http_respuesta_ok(codigo, tipo.datos, cuerpo.datos, cuerpo.longitud);
