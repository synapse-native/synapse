...
- **Estado R11:** ✅ **CERRADA** (fix `fe5e7aa`, 2026-08-10) — exhaustividad `coincidir` nativa cableada (ver sección 2). Diagnóstico observable `faltan variantes ok/err`, ejecución real del switch 42/0, bootstrap S2==S3 (md5 `c17e4658`), 25/25 HM, regresión 176 passed.
...
- [x] R11 — exhaustividad nativa cableada: `NodoCoincidir` en flatten F8 + parser/ast_nodes/puente/analizador (buffers por puntero)/generador (switch `.tag`)/D-2 (parámetros) — commit `fe5e7aa`, bootstrap `c17e4658`, 25/25 HM, 176 passed
...
2. **R11 — exhaustividad nativa** ✅ **CERRADA** (2026-08-10, fix `fe5e7aa`, docs en el mismo tramo): cableado completo del `coincidir` (flatten F8 + parser + analizador con buffers por puntero + generador switch `.tag` + D-2 parámetros); diagnóstico observable, ejecución real 42/0, bootstrap `c17e4658`, 25/25 HM, 176 passed.
