...
- **PLAN DE MICRO-ENTREGABLES (2026-08-27, PENDIENTE DE APROBACIÓN DEL ARQUITECTO):** Plan para acercar el código a los manuales v8.1.0 solo donde la divergencia es carencia real (no forzar nombres de archivo ni builtins embebidas). Criterio de selección: (a) mejora real del código, (b) cumple manual. MEs:
  - **ME-1 — `std/os.syn` (M9 §5.7 / F29):** ✅ CERRADO. Wrapper Synapse FFI sobre `runtime/core/detect_hardware.c` (`memoria_total`, `memoria_libre`, `vram_total`, `cpu_nucleos`, `arquitectura`). Archivos: `detect_hardware.h`, `detect_hardware.c`, `std/os.syn`, `tests/unit/test_os_syn.py`. Tests: 5/5 PASS. Auditoría: 0 brechas. Commit pendiente.
  - **ME-2 — Cierre LSP F27 + comandos IA (M8 §1, ANEXO-MANUALES §1.4):** `nucleo/lsp_v3.syn` 7/9→9/9 + dispatch `synapse/aiComplete`/`aiFix`/`aiTranspile` stubs. Tests: 26/26 PASS, 0 fallos, 0 skipped. Fix: didOpen/didChange emiten publishDiagnostics con check #lang (ERR_LANG_MISSING); shutdown con id (request); dispatch con error -32601 para metodo desconocido. Commits: `3bcf575` + pendiente. Estado: ✅ CERRADO.
  - **ME-3 — Contratos `requiere/garantiza` en `std/` (Manual 2 §12):** ✅ CERRADO. Contratos agregados a 4 archivos (io: 6 funcs, json: 5, math: 4, texto: 16 = 31 funciones total). Tests TDD: 20/20 PASS (`tests/unit/test_std_contracts.py`). Auditoría: 0 brechas. Commit pendiente.
  - **ME-4 — Fortalecer oraculos de test criticos (M7 §2.3, M3 §12.1): ✅ CERRADO. 165/165 tests convertidos de content/smoke a oraculos reales (LOTES 1-12, commits 2fb58bd-6ceb4c0). 0 skips restantes. Commit pendiente.
...
- **PLAN DE ME DE SEGURIDAD (2026-08-27, PENDIENTE DE APROBACIÓN):** reparaciones alineadas a manuales.
  - **ME-SEC-1 — `_json_a_texto` sin buffer estatico (Manual 4 §2.1): ✅ CERRADO. pool_alloc copy, >64KB retorna vacio. Tests TDD 3/3 PASS. Commit pendiente.
  - **ME-SEC-2 — Validacion protocolo LSP (Manual 8 §1.2): ✅ CERRADO. Content-Length bounds + JSON parse validation + construir_error snprintf fix. Tests TDD 5/5, LSP suite 31/31. Commit pendiente.
  ...
  - **ME-SEC-4 — Estado estático → fiber-safe (Manual 4 §2.1, Manual 5):** `io.c _buf`, `texto.c _split_store`, `modelo.c _cached_codigo_c` → TLS/fiber-local o mutex, o documentar single-thread. `_ser_buf` ya cubierto por ME-SEC-1 — `_json_a_texto` sin buffer estatico (Manual 4 §2.1): ✅ CERRADO. pool_alloc copy, >64KB retorna vacio. Tests TDD 3/3 PASS. Commit pendiente.
