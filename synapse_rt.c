...
#include "runtime/core/cluster.h"  // D-9(d): std.cluster (M8.1-M8.6) extraido a cluster.c#include "runtime/core/debug.h"  // D-9(d): debug (M9.0-M9.4) extraido a debug.c
...
CadenaSegura entero_a_texto(int64_t n) {
    ...
    memcpy(data, buf, len + 1);
    return (CadenaSegura){ .longitud = len, .datos = data };
}// ============================================================// D-9(d) corte 7: std.json extraido a runtime/core/json.c (texto byte-identico)
// ============================================================
