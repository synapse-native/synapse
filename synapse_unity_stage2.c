...
void gen_visitar_para(struct GeneradorCEst est, void* stmt);
...
void gen_visitar_si(struct GeneradorCEst est, void* stmt) {
    { /* unsafe */
        ...
        // [PGO] Counter at if-true block entry;
        ...
        if (_s->cuerpo_sino) {
            ...
            gen_abrir_bloque_c(est);
            // [PGO] Counter at if-false (else) block entry;
            pgo_instrumentar_if_false(est);
...
void gen_visitar_mientras(struct GeneradorCEst est, void* stmt) {
    { /* unsafe */
        ...
        gen_emitir_str(est, _buf);
        gen_abrir_bloque_c(est);
        // [PGO] Counter at while-body entry;
        pgo_instrumentar_while_body(est);
...
void gen_visitar_para(struct GeneradorCEst est, void* stmt) {
    { /* unsafe */
        struct SentenciaPara* _p = (struct SentenciaPara*)stmt;
        if (!_p) return;
        gen_emitir_traza(est, "nodos_flujo.syn", "SentenciaPara");
        char _buf[4096], _init[512], _cond[512], _step[512];
        _init[0] = _cond[0] = _step[0] = 0;
        // Evaluate init, cond, step expressions;
        if (_p->inicializacion) { _oo_expr_a_c(est, _p->inicializacion, _init, 512); }
        if (_p->condicion) { _oo_expr_a_c(est, _p->condicion, _cond, 512); }
        if (_p->incremento) { _oo_expr_a_c(est, _p->incremento, _step, 512); }
        // Emit: for (init; cond; step) {
        snprintf(_buf, 4096, "    for (%s; %s; %s)", _init, _cond, _step);
        gen_emitir_str(est, _buf);
        gen_abrir_bloque_c(est);
        // [PGO] Counter at for-body entry;
        pgo_instrumentar_for_body(est);
        // For-body via dispatcher;
        {
            struct Nodo* _fc = _p->cuerpo;
            if (_fc) { gen_visitar_stmt_generico(est, _fc); }
        }
        gen_cerrar_bloque_c(est);
    }
}
...
void gen_visitar_stmt_generico(struct GeneradorCEst est, void* stmt) {
    { /* unsafe */
        ...
        else if (_syn_es_tipo(_t, "SentenciaPara")) { gen_visitar_para(est, stmt); }
...
void gen_visitar_top_level(struct GeneradorCEst est, void* stmt) {
    { /* unsafe */
        ...
        if (_syn_es_tipo(_t, "DefinicionFuncion")) {
            ...
            int _base_pgo = pgo_emitir_contadores_funcion(est, 1);
            pgo_emitir_incremento_id(est, _base_pgo);
...
struct ResultadoEtapa etapa_ok(void) {
    ...
    r.tag = 0;
    struct ResultadoEtapa _ret_26 = r;
    return _ret_26;
}
...
struct ResultadoEtapa generar_etapa(CadenaSegura ruta, CadenaSegura salida) {
    { /* unsafe */
        ...
        int _is_usr = (ruta.longitud > 0 && ruta.datos != NULL && ruta.datos[0] != 0) ? 1 : 0;
        if (_is_usr) {
            fprintf(stderr,"[Synapse] Compilando archivo: %s...\n",ruta.datos);
            _P_ntks=0;_P_tpos=0;_P_p_err=0;
            FILE* _f=fopen(ruta.datos,"rb"); if(!_f){struct ResultadoEtapa _e={1,2};return _e;}
            fseek(_f,0,SEEK_END);long _fsz=ftell(_f);fseek(_f,0,SEEK_SET);
            if(_fsz>1048576){fprintf(stderr,"FATAL: %s >1MB\n",ruta.datos);fclose(_f);struct ResultadoEtapa _e={1,6};return _e;}
            char* _buf=(char*)malloc((size_t)(_fsz<1048576?1048576:_fsz)+1);
            fread(_buf,1,(size_t)_fsz,_f);fclose(_f);_buf[_fsz]=0;
            CadenaSegura _fuente={.longitud=(int)_fsz,.datos=_buf};
            int _ntok=tokenizar(_fuente); if(_ntok<0){free(_buf);struct ResultadoEtapa _e={1,3};return _e;}
            struct Programa _prog=parsear(_fuente); free(_buf);
            struct ListaNodo* _cur=_prog.sentencias;
            while(_cur){if(_cur->cabeza){*_mcur=(struct ListaNodo*)calloc(1,sizeof(struct ListaNodo));(*_mcur)->cabeza=_cur->cabeza;(*_mcur)->cola=NULL;_mcur=&(*_mcur)->cola;}_cur=_cur->cola;}
        } else {
            fprintf(stderr,"[Synapse] Pipeline nativa: Unity Build - leyendo nucleo...\n");
    ...
    struct ResultadoEtapa _ret_236 = etapa_ok();
    return _ret_236;
}
            ...
            for(int _ti=0;_ti<_rt.longitud;_ti++){
                if(strcmp(_rt.pares[_ti].clave.datos,"dependencias")==0){
                    ...
                    for(int _tj=0;_tj<_dep->longitud;_tj++){
                        ...
                        // Step 1.5: Verify Ed25519 (mandatory: zero-tolerance, before lock/extract);
                        char sig_path[512];
                        FILE* _sf=NULL;
                        // Try .sig at: 1) alongside tar_path (paquetes_oficiales/), 2) .axon_cache/<pkg>.tar.sig, 3) HTTP;
                        snprintf(sig_path,sizeof(sig_path),"%s.sig",_tar_path); _sf=fopen(sig_path,"rb");
                        if(!_sf){ snprintf(sig_path,sizeof(sig_path),".axon_cache/%s.tar.sig",_pkg.datos); _sf=fopen(sig_path,"rb"); }
                        if(!_sf && _online){
                            char sig_url[512];
                            ...
                            if(_syn_http_get_archivo(_gh_host_sig,443,_gh_path_sig,sig_path)>=0) _sf=fopen(sig_path,"rb");
                        }
                        // Zero-tolerance: if autor is set but no .sig, abort;
                        if(!_sf && *autor){ fprintf(stderr,"[Axon] ERR_AXON_COMPROMISED: .sig no encontrado\n"); remove(_tar_path); r=1; free(_b); return r; }
                        // Mandatory verification before lock+extract;
                        if(_sf){ fclose(_sf); fprintf(stderr,"[Axon] Verificando firma Ed25519...\n"); int _vf=_syn_axon_verificar_firma(_tar_path,sig_path,autor); if(_vf<0){ fprintf(stderr,"[Axon] ERR_AXON_COMPROMISED: firma Ed25519 invalida\n"); remove(_tar_path); remove(sig_path); r=1; free(_b); return r; } fprintf(stderr,"[Axon] Firma Ed25519 verificada correctamente\n"); }
