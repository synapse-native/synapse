...
// Returns:
// --- SemVer helper: parse major.minor.patch from version string ---
// Returns 1 on success, 0 on parse failure.
static int _syn_parse_ver(const char* v, int* maj, int* min, int* pat) {
    if (sscanf(v, "%d.%d.%d", maj, min, pat) == 3) return 1;
    // Try with leading 'v': v1.2.3
    if (v[0] == 'v' || v[0] == 'V') {
        if (sscanf(v + 1, "%d.%d.%d", maj, min, pat) == 3) return 1;
    }
    return 0;
}
...
// Supports:
//   "1.2.3"     - exact version
//   "^1.2.3"    - compatible: >=1.2.3 and <next-major (or <next-minor for 0.x)
//   "~1.2.3"    - patch-level: >=1.2.3 and <next-minor
...
int _syn_semver_match(const char* constraint, const char* version) {
    ...
    if (prefix == '^') {
        ...
        // ^0.0.3 means >=0.0.3 and <0.0.4  (i.e. exact 0.0.3)
        ...
        // Compare using encoded integer for simplicity
...
int _syn_axon_buscar_local(const char* paquete, const char* version,
    ...
    // 3. Check paquetes_oficiales/<pkg>/ — try exact path first, then SemVer scan
    // 3a. Try exact version path (fast path for "1.2.3" without prefix)
    snprintf(tar_path, tar_sz, "paquetes_oficiales/%s/%s.tar", paquete, version);
    f = fopen(tar_path, "rb");
    if (f) { fclose(f); fprintf(stderr, "[Axon] Local: oficial %s\n", tar_path); return 2; }

    // 3b. If constraint is ^ or ~, or exact path failed, scan directory with SemVer
    char pkg_dir[1024];
    snprintf(pkg_dir, sizeof(pkg_dir), "paquetes_oficiales/%s", paquete);
    ...
    if (dir) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (strcmp(name + namelen - 4, ".tar") != 0) continue;
            // Extract version from "X.Y.Z.tar"
            char ver_candidate[64];
            int vlen = namelen - 4;
            if (vlen >= (int)sizeof(ver_candidate)) continue;
            memcpy(ver_candidate, name, vlen);
            ver_candidate[vlen] = '\0';
            // Check if it matches the SemVer constraint
            if (_syn_semver_match(version, ver_candidate)) {
                // Pick the highest matching version (use encoded comparison)
                int vmaj = 0, vmin = 0, vpat = 0;
                if (_syn_parse_ver(ver_candidate, &vmaj, &vmin, &vpat)) {
                    long long cur = (long long)vmaj * 1000000 + vmin * 1000 + vpat;
                    long long best = (long long)best_maj * 1000000 + best_min * 1000 + best_pat;
                    if (!*best_match || cur > best) {
                        strncpy(best_match, ver_candidate, sizeof(best_match) - 1);
                        best_match[sizeof(best_match) - 1] = '\0';
                        best_maj = vmaj; best_min = vmin; best_pat = vpat;
                    }
                }
            }
        }
        closedir(dir);
        if (*best_match) {
            snprintf(tar_path, tar_sz, "paquetes_oficiales/%s/%s.tar", paquete, best_match);
            fprintf(stderr, "[Axon] Local: oficial %s (SemVer match: %s)\n", tar_path, version);
            return 2;
        }
    }
